...
connexion.addEventListener("submit", function (e) {
    ...
    fetch("http://localhost:5678/api/users/login", {
        ...
        .then(response => {
            ...
            } else{ //if (response.status === 401) { //retourne un message d'erreur 
                //document.getElementById("messageerreur").textContent = "Erreur dans l’identifiant ou le mot de passe";
                //document.getElementById("messageerreur").style.display = "block";
                //return;
                throw new Error("Erreur dans l’identifiant ou le mot de passe");

            }// else if (response.status === 404) {//retourne un message d'erreur 
                //document.getElementById("messageerreur").textContent = "Erreur dans l’identifiant ou le mot de passe";
               // document.getElementById("messageerreur").style.display = "block";
               // return;
            //}
        ...
        .catch(error =>{
              document.getElementById("messageerreur").textContent = error;
                document.getElementById("messageerreur").style.display = "block";
        }
        );
